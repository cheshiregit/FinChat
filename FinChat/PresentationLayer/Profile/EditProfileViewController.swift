...
class EditProfileViewController: UIViewController, UIImagePickerControllerDelegate,
    ...
    func showErrorAlert(savingFunc: @escaping () -> Void) {
        let errorAlert = UIAlertController(title: "Ошибка", message: "Не удалось сохранить данные", preferredStyle: .alert)

        let okAction = UIAlertAction(title: "OK", style: .default)

        let repeatAction = UIAlertAction(title: "Повторить", style: .destructive) { (_: UIAlertAction!) -> Void in
            savingFunc()
        }
    ...
    override func viewDidLoad() {
        ...
        self.refreshData()
        self.buttonsEnabled(state: false)
        NotificationCenter.default.addObserver(self, selector: #selector(textViewEnabled),
                                               name: UITextView.textDidChangeNotification, object: aboutUserTextView)
