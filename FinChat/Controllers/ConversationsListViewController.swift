...
extension ConversationsListViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let tableViewCell = tableView.dequeueReusableCell(withIdentifier: "cellReuseIdentifier", for: indexPath) as? ConversationCellConfiguratioin
        if let cell = tableViewCell {
            ...
            if let returnCell = cell as? UITableViewCell {
                return returnCell
            } else {
...
//        print(segue.destination) themesSegue
        ...
        case "showConversation":
            guard let viewController = segue.destination as? ConversationViewController else { return }
            guard let dialog = sender as? CellModel? else { return }
            viewController.conversationTitle = dialog?.name
            viewController.dialog = dialog
            viewController.delegate = self
            viewController.idUserTo = dialog?.userID
            viewController.messages = (dialog?.message)!
        case "themesSegue":
            guard let viewController = segue.destination as? ThemesViewControllerSwift else { return }
            viewController.themesClosure = { theme in
